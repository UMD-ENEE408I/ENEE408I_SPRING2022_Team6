...
void PIDForward(Encoder &encL, Encoder &encR) {
  /*PID Control*/
  float targetVel_L = 10;  //in cm/s
  float targetVel_R = 10;  //in cm/s
  float targetPos_L = 0;
  ...
  while (1) {
    ...
    senseLine(bit_buf);
    if(leftSensors > 5 || rightSensors > 5) {//junction detected
...
void PIDForwardDist(float distance, Encoder &encL, Encoder &encR) {
  /*PID Control*/
  float targetVel_L = 10;  //in cm/s
  float targetVel_R = 10;  //in cm/s
  float targetPos_L = 0;
...
int detectJunction(int leftSensors, int rightSensors) {
  senseLine(bit_buf);
  if(leftSensors >= 4 && rightSensors >= 4) {
    Serial.println("FULL"); //all sensors lit
    return 1;
    //could be T or +, drive forward to test
    //PIDForward
    //senseLine
    //if bit_buf[7] == 0, then it's a T
    //backwards without PID
  }
  else if(leftSensors >= 5) {
    Serial.println("LEFT");
    return 2;
  }
  else if(rightSensors >= 5) {
    Serial.println("RIGHT");
    return 3;
  }
  else if(bit_buf[7] == 1 && (leftSensors >= 1 && leftSensors <= 4) && (rightSensors >= 1 && rightSensors <= 4)) {
...
void loop() {
  ...
  while (1) {
    result = detectJunction(leftSensors, rightSensors);
    switch (result) {
      case 1: //array FULL, T or +
        PIDForwardDist(8, encL, encR); //go past to make room for turn and check again
        result = detectJunction(leftSensors, rightSensors);
        if(result == 0) { //it's a +
          rotateLeft(90, encL, encR);
        }
        else {
          rotateLeft(90, encL, encR);
        }
        break;
      case 2: //LEFT or -|
        PIDForwardDist(8, encL, encR); //go past to make room for turn
        result = detectJunction(leftSensors, rightSensors);
        if(result == 0) { //it's a -|
          rotateLeft(90, encL, encR);
        }
      ...
      case 3: //RIGHT or |-
        ...
        result = detectJunction(leftSensors, rightSensors);
        if(result == 0) { //it's a |-
          PIDForward(encL, encR);
        }
        else {
        rotateRight(90, encL, encR);
        }
        break;
      case 4: //END
        stopMove();
        break;
      case 5: //NOTHING, DEAD END
        PIDForwardDist(5, encL, encR); //go past to make room for turn
        rotateRight(180, encL, encR);
        break;
      default: //LINE
        PIDForward(encL, encR);
        break;
